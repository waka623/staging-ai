...
async function generateReportPDF() {
  ...
  try {
    ...
    // 【修正】グラデーション、見出しの改行防止、文言を完全に反映したテンプレート
    const htmlContent = `
    <!DOCTYPE html>
    <html lang="ja">
    <head>
        ...
        <style>
            ...
            .title { color: #1A202C; font-size: 24px; margin: 0; font-weight: bold; }
            .url { color: #718096; font-size: 14px; margin-top: 5px; }

            .summary-container { display: flex; gap: 20px; margin-bottom: 30px; }
            .card { flex: 1; border-radius: 12px; padding: 24px; color: white; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05); }

            /* 【修正】コスト側カードを綺麗なグラデーションに変更 */
            .card-cost { background: linear-gradient(135deg, #1A365D, #2B6CB0); }
            .card-co2 { background: linear-gradient(135deg, #2F855A, #38A169); }

            .card-label { font-size: 14px; opacity: 0.9; font-weight: 500; }
            .card-value { font-size: 30px; font-weight: bold; margin: 12px 0; }
            ...
            /* 【修正】表の見出しが途中で改行されるのを完全に防ぐ */
            th { background-color: #EDF2F7; text-align: left; padding: 14px; font-size: 13px; color: #4A5568; white-space: nowrap; }

            td { padding: 14px; border-bottom: 1px solid #E2E8F0; font-size: 13.5px; line-height: 1.5; }
    ...
    <body>
        <div class="header">
            ...
            <!-- 文言要件を満たしたヘッダー -->
            <div style="text-align: right; font-size: 12px; color: #718096; white-space: nowrap;">経済産業省ガイドライン ver.2.7準拠</div>
        </div>

        <div class="summary-container">
            <div class="card card-cost">
                <div class="card-label">💰 年間の既存固定費 削減見込み額</div>
                <div class="card-value">￥${esc(data.annual_saved_cost)} <span style="font-size: 14px;">/ 年</span></div>
                <!-- 文言要件：「翌月から浮きます」を反映 -->
            ...
            <div class="card card-co2">
                ...
                <div class="card-value">${esc(data.saved_co2_monthly)} <span style="font-size: 14px;">kg-CO2 / 月</span></div>
                <!-- 文言要件：「Scope 3提出データとしてそのまま利用可能」を反映 -->
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th class="text-right">削減金額 / 月</th>
                    <th class="text-right">削減CO2 / 月</th>
                </tr>
            </thead>
            <tbody>
                <tr>
                    <td><strong>① Webフロントエンド</strong><br><span style="font-size: 11px; color: #718096;">(ECサイト通信量削減)</span></td>
                    <td>ページサイズ: ${esc(data.current_page_size)}<br>月間 ${esc(data.monthly_access)}回</td>
                ...
                <tr>
                    <td><strong>② 拠点エネルギー</strong><br><span style="font-size: 11px; color: #718096;">(オフィス Scope 2)</span></td>
