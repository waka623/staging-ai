...
不動産仲介・管理会社向けのSaaS。空室の写真数枚と間取り図をアップロードすると、AIが部屋の形状を読み取り、家具配置プランを3パターン提案する。

競合の多くは「写真に家具をCG合成してリアルに見せる」フォトリアル方式だが、本サービスは間取り図から部屋の実寸を読み取り、家具も実寸（メートル単位）で配置する設計のため、「見た目」だけでなく「実際にその通りに置けること」を保証できる点が差別化ポイント。要件定義の詳細は別途まとめたメモを参照。
...
## デモモード（Supabase/Anthropicなしで動作確認）

Supabaseプロジェクトや Anthropic APIキーを用意していなくても、`DEMO_MODE=1` を付けて起動すると、認証・DB・AI解析をすべてインメモリのダミーデータに差し替えた状態でUI全体を確認できる。

```bash
DEMO_MODE=1 npm run dev
```

サインアップ・ログイン不要で `/dashboard` にサンプル物件が表示され、「新規物件登録」からファイルをアップロードすると、実際のAI解析の代わりに固定のレイアウト案が数秒で生成される（`src/lib/demo/`）。共有リンクも実際に発行・閲覧できる。画面上部に「デモモード」のバナーが常時表示される。`DEMO_MODE` を外せば通常どおり実際のSupabase/Claudeを使う実装に戻る。
